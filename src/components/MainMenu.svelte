...
<script>
	...
	let menuItems = [
		...
		{ name: 'Demo Classes', link: '/demo-classes' },
...
<header class="flex items-center justify-between bg-white px-6 py-3 shadow">
	...
	<!-- Desktop Navigation -->
	<nav class="hidden space-x-6 text-gray-700 md:flex">
		{#each menuItems as item, index}
			...
			>
				<a
					href={item.link}
					class="nav-link relative block rounded-md px-3 py-2 transition-all duration-300"
               ...
               color: {activeItem === index ? alpineGreen : 'inherit'};"
				>
					...
					<span
						class="nav-indicator absolute bottom-0 left-1/2 block h-0.5 w-0 -translate-x-1/2 transform bg-[#036752] transition-all duration-300"
						style="width: {activeItem === index ? '80%' : '0'}; opacity: {activeItem === index
							? '1'
							: '0'}"
					></span>
				</a>
				{#if item.dropdown}
					<div
						class="absolute left-0 z-2 mt-2 w-48 overflow-hidden rounded-md bg-white shadow-lg"
						style="opacity: {activeItem === index ? '1' : '0'}; pointer-events: {activeItem ===
						index
							? 'all'
							: 'none'};
                 box-shadow: 0 4px 15px rgba(3, 103, 82, 0.2), 0 1px 3px rgba(3, 103, 82, 0.1);"
					...
					>
						<a
							href="#"
							class="nav-dropdown-item block transform px-4 py-3 transition-all duration-200 hover:translate-x-1"
							>Option 1</a
						>
						<a
							href="#"
	...
	<div class="hidden md:block">
		<a
			...
			class="lets-talk-btn relative inline-block overflow-hidden rounded-full px-6 py-2.5 font-medium text-white transition-all duration-300"
		...
		>
			...
			<div class="absolute inset-0 rounded-full bg-gradient-to-r from-[#036752] to-[#0d8b75]"></div>
			...
			{#if buttonHovered}
				<div
					class="absolute inset-0 rounded-full bg-white opacity-10"
...
{#if menuOpen}
	<div class="fixed top-0 right-0 z-2 h-screen" transition:fly={{ x: 200, duration: 500 }}>
		<div class="flex h-full w-64 flex-col space-y-4 bg-white p-6 shadow-lg">
			<!-- Close button -->
			...
			{#each menuItems as item, index}
				...
				>
					<a
						href={item.link}
						class="mobile-nav-item block rounded-md border-b border-gray-100 px-3 py-3 transition-all duration-200"
					...
					{#if item.dropdown}
						<div class="mt-2 hidden space-y-2 pl-4 group-hover:block">
							<a
								href="#"
								class="mobile-dropdown-item my-2 block transform rounded-md px-3 py-2 transition-all duration-200 hover:translate-x-1"
								style="box-shadow: 0 1px 2px rgba(3, 103, 82, 0.1);"
								on:click={closeMenu}>Option 1</a
							>
							<a
								href="#"
								class="mobile-dropdown-item my-2 block transform rounded-md px-3 py-2 transition-all duration-200 hover:translate-x-1"
								style="box-shadow: 0 1px 2px rgba(3, 103, 82, 0.1);"
								on:click={closeMenu}>Option 2</a
							>
						</div>
					{/if}
				</div>
			...
			<a
				...
				target="_blank"
				rel="noopener noreferrer"
				class="lets-talk-btn relative mt-4 inline-block w-full overflow-hidden rounded-full px-6 py-2.5 text-center font-medium text-white transition-all duration-300"
			...
			>
				<div
					class="absolute inset-0 rounded-full bg-gradient-to-r from-[#036752] to-[#0d8b75]"
				></div>
				<div class="relative z-10 flex items-center justify-center space-x-1">
					<span>Let's Talk</span>
					<svg
